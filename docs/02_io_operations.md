# IO Operations: Connecting, Reading, and Exporting Data

## Database Connections (The `connect` function)
...
## Exporting Data (the `export_table` function)
...
## Return Data as DataFrame (the `DF` function)

The `DF` function is your primary tool for retrieving data from the DuckDB engine into a Python-friendly **Pandas DataFrame**.

While `pysqdb` handles heavy computations at C++ speeds within the database, `DF` allows you to pull specific slices of your data into RAM for final visualization, machine learning, or custom Python processing.

`ps.DF(table, head=None, tail=None, sample=None, columns=None)`
...
## Return Data as Array (the `give_array` function)
...
## Get information about the table (the `give_info` function)
...
## Convert DataFrame to Table (the `import_df` function)
...
The simplest way to move your work from Pandas to the SQL power of `pysqdb`.
...
### 2. Converting a Specific Subset (Slicing)
...
## Copy the Table (the `copy_table` function)
